...
use App\Models\Sale_orders;

use App\Repositories\Sale_ordersRepositoryInterface;
use Illuminate\Support\Collection;
use Illuminate\Support\Facades\DB;

class Sale_ordersRepository extends BaseRepository implements Sale_ordersRepositoryInterface
{
    public function __construct(Sale_orders $model)
    {
    ...
    public function create(array $attributes, $currentUser)
    {
        try {
            ...
            $sale_orders = Sale_orders::create([
    ...
    public function update($id, array $attributes, $currentUser)
    {
        try {
            $attributes = $attributes['data'];
            DB::beginTransaction();
            $sale_orders = Sale_orders::find($id);
            $sale_orders->update([
