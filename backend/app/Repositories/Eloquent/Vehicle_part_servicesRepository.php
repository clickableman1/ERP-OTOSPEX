...
use App\Models\Vehicle_part_services;

use App\Repositories\Vehicle_part_servicesRepositoryInterface;
use Illuminate\Support\Collection;
use Illuminate\Support\Facades\DB;

class Vehicle_part_servicesRepository extends BaseRepository implements Vehicle_part_servicesRepositoryInterface
{
    public function __construct(Vehicle_part_services $model)
    {
    ...
    public function create(array $attributes, $currentUser)
    {
        try {
            ...
            $vehicle_part_services = Vehicle_part_services::create([
    ...
    public function update($id, array $attributes, $currentUser)
    {
        try {
            $attributes = $attributes['data'];
            DB::beginTransaction();
            $vehicle_part_services = Vehicle_part_services::find($id);
            $vehicle_part_services->update([
