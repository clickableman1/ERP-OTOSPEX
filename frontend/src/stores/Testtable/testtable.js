...
export const useTesttableStore = defineStore('testtable', {
  ...
  actions: {
    async fetch(id = '', query) {
      this.startLoading()
      try {
        const result = await axios.get(`testtable${query || (id ? `/${id}` : '')}`);
    ...
    async deleteItem(id) {
      try {
        await axios.delete(`/testtable/${id}`);
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/testtable', { data: payload });
        this.showNotification('Testtable has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/testtable/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Testtable has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
