...
import TableSampleInventory_history from '@/components/Inventory_history/TableSampleInventory_history.vue'
...
const getInventory_historyCSV = async () => {
  const response = await axios({url: '/inventory_history?filetype=csv', method: 'GET',responseType: 'blob'});
  const type = response.headers['content-type']
  const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
  const link = document.createElement('a')
  link.href = window.URL.createObjectURL(blob)
  link.download = 'inventory_historyCSV.csv'
  link.click()
}
...
const parseCsv = async () => {
  try {
    ...
    const result = await axios.post('inventory_history/bulk-import', data, {
      headers: {
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    inventory_historyStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Inventory_history'
    : 'http://localhost:8080/api-docs/#/Inventory_history';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Inventory_history</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
        label="New"
        to="/inventory_history/new"
      />
      <BaseButton
        color="default"
        label="Add Filter"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for inventory_history</a>

    <CardBox
      class="mb-6"
      title="Inventory_history table"
      has-table
    >
      <TableSampleInventory_history checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
