...
import TableSampleSettings from '@/components/Settings/TableSampleSettings.vue'
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    settingsStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Settings'
    : 'http://localhost:8080/api-docs/#/Settings';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Settings</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for settings</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleSettings checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
