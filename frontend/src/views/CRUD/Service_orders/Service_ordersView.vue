...
import TableSampleService_orders from '@/components/Service_orders/TableSampleService_orders.vue'
...
const getService_ordersCSV = async () => {
  const response = await axios({url: '/service_orders?filetype=csv', method: 'GET',responseType: 'blob'});
  const type = response.headers['content-type']
  const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
  const link = document.createElement('a')
  link.href = window.URL.createObjectURL(blob)
  link.download = 'service_ordersCSV.csv'
  link.click()
}
...
const parseCsv = async () => {
  try {
    ...
    const result = await axios.post('service_orders/bulk-import', data, {
      headers: {
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    service_ordersStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Service_orders'
    : 'http://localhost:8080/api-docs/#/Service_orders';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Service_orders</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
        label="New"
        to="/service_orders/new"
      />
      <BaseButton
        color="default"
        label="Add Filter"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for service_orders</a>

    <CardBox
      class="mb-6"
      title="Service_orders table"
      has-table
    >
      <TableSampleService_orders checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
