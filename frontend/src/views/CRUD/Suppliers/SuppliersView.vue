...
import TableSampleSuppliers from '@/components/Suppliers/TableSampleSuppliers.vue'
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    suppliersStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Suppliers'
    : 'http://localhost:8080/api-docs/#/Suppliers';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Suppliers</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for suppliers</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleSuppliers checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
