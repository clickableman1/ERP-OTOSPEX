...
import TableSampleServices from '@/components/Services/TableSampleServices.vue'
...
const getServicesCSV = async () => {
  const response = await axios({url: '/services?filetype=csv', method: 'GET',responseType: 'blob'});
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    servicesStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Services'
    : 'http://localhost:8080/api-docs/#/Services';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Services</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for services</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleServices checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
