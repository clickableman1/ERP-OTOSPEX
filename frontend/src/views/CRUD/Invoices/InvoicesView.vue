...
import TableSampleInvoices from '@/components/Invoices/TableSampleInvoices.vue'
...
const getInvoicesCSV = async () => {
  const response = await axios({url: '/invoices?filetype=csv', method: 'GET',responseType: 'blob'});
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    invoicesStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Invoices'
    : 'http://localhost:8080/api-docs/#/Invoices';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Invoices</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for invoices</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleInvoices checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
