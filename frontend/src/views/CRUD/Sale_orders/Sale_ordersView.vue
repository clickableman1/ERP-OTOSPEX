...
import TableSampleSale_orders from '@/components/Sale_orders/TableSampleSale_orders.vue'
...
const getSale_ordersCSV = async () => {
  const response = await axios({url: '/sale_orders?filetype=csv', method: 'GET',responseType: 'blob'});
  const type = response.headers['content-type']
  const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
  const link = document.createElement('a')
  link.href = window.URL.createObjectURL(blob)
  link.download = 'sale_ordersCSV.csv'
  link.click()
}
...
const parseCsv = async () => {
  try {
    ...
    const result = await axios.post('sale_orders/bulk-import', data, {
      headers: {
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    sale_ordersStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Sale_orders'
    : 'http://localhost:8080/api-docs/#/Sale_orders';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Sale_orders</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for sale_orders</a>

    <CardBox
      class="mb-6"
      title="Sale_orders table"
      has-table
    >
      <TableSampleSale_orders checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
