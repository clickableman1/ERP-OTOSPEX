...
import TableSampleTesttable from '@/components/Testtable/TableSampleTesttable.vue'
...
const filters = ref([]);
const filtersList = [{label: 'Testfield', title: 'testfield'},
...
const getTesttableCSV = async () => {
  const response = await axios({url: '/testtable?filetype=csv', method: 'GET',responseType: 'blob'});
  const type = response.headers['content-type']
  const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
  const link = document.createElement('a')
  link.href = window.URL.createObjectURL(blob)
  link.download = 'testtableCSV.csv'
  link.click()
}
...
const parseCsv = async () => {
  try {
    ...
    const result = await axios.post('testtable/bulk-import', data, {
      headers: {
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    testtableStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Testtable'
    : 'http://localhost:8080/api-docs/#/Testtable';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Testtable</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for testtable</a>

    <CardBox
      class="mb-6"
      title="Testtable table"
      has-table
    >
      <TableSampleTesttable checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
