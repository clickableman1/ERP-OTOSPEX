...
import TableSamplePurchase_orders from '@/components/Purchase_orders/TableSamplePurchase_orders.vue'
...
const getPurchase_ordersCSV = async () => {
  const response = await axios({url: '/purchase_orders?filetype=csv', method: 'GET',responseType: 'blob'});
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    purchase_ordersStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Purchase_orders'
    : 'http://localhost:8080/api-docs/#/Purchase_orders';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Purchase_orders</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for purchase_orders</a>

    <CardBox
      class="mb-6"
      title="Purchase_orders table"
      has-table
    >
      <TableSamplePurchase_orders checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
