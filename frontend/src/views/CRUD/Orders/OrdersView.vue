...
import TableSampleOrders from '@/components/Orders/TableSampleOrders.vue'
...
const getOrdersCSV = async () => {
  const response = await axios({url: '/orders?filetype=csv', method: 'GET',responseType: 'blob'});
...
const parseCsv = async () => {
  try {
    ...
    const result = await axios.post('orders/bulk-import', data, {
      headers: {
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    ordersStore.fetch(null, query.value);
...
<CardBoxModal
  ...
  <SectionHeroBar>Orders</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for orders</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleOrders checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
