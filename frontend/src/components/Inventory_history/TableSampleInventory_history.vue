...
inventory_historyStore.fetch()

/* Delete item */
const deleteItem = () => {
  inventory_historyStore.deleteItem(currentId.value)
}

const items = computed(() => inventory_historyStore.data)
  ...
  <table>
    ...
    <tbody>
      <tr
        ...
        :key="client.id"
        @click="$router.push(`/inventory_history/${client.id}/edit`)"
      >
        <TableCheckboxCell
          v-if="checkable"
        ...
        <td class="before:hidden lg:w-1 whitespace-nowrap">
          ...
          >
            <BaseButton
              ...
              :icon="mdiEye"
              small
              :to="`/inventory_history/${client.id}/edit`"
            />
            <BaseButton
              color="danger"
              :icon="mdiTrashCan"
