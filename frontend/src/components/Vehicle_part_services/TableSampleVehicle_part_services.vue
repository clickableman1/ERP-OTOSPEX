...
vehicle_part_servicesStore.fetch()

/* Delete item */
const deleteItem = () => {
  vehicle_part_servicesStore.deleteItem(currentId.value)
}

const items = computed(() => vehicle_part_servicesStore.data)
...
<template>
  <CardBoxModal
    ...
    title="Vehicle_part_services modal"
  >
    <ul
  ...
  <table>
    ...
    <tbody>
      <tr
        ...
        :key="client.id"
        @click="$router.push(`/vehicle_part_services/${client.id}/edit`)"
      >
        <TableCheckboxCell
          v-if="checkable"
        ...
        <td class="before:hidden lg:w-1 whitespace-nowrap">
          ...
          >
            <BaseButton
              ...
              :icon="mdiEye"
              small
              :to="`/vehicle_part_services/${client.id}/edit`"
            />
            <BaseButton
              color="danger"
              :icon="mdiTrashCan"
