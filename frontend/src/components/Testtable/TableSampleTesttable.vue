...
testtableStore.fetch()

/* Delete item */
const deleteItem = () => {
  testtableStore.deleteItem(currentId.value)
}

const items = computed(() => testtableStore.data)

const headers = [

{ text: 'Testfield', value: 'testfield'},]
...
<template>
  ...
  <table>
    <thead>
      ...
      <th
        :class="['sortable uppercase text-sm font-normal text-pavitra-600', props.sortTitle === 'testfield' && props.sortDirection]"
        @click="sort('testfield')"
      >Testfield</th>
    ...
    <tbody>
      <tr
        ...
        :key="client.id"
        @click="$router.push(`/testtable/${client.id}/edit`)"
      >
        <TableCheckboxCell
          v-if="checkable"
